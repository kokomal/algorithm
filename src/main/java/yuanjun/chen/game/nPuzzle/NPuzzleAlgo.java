 ...
 * @Description: IDA*解决nPuzzle问题 在线试运算请参见http://tristanpenman.com/demos/n-puzzle/
 * @author: 陈元俊
 * @date: 2018年10月25日 下午3:01:28
 */
@Deprecated
public class NPuzzleAlgo {
    ...
    public void generateRandomGame(int n, int hard) {
        ...
        do {
            ...
            fillInGrids(N, this.rawList, this.grids);
        } while (!solvable() || manhattan(false, MODEL_GRIDS, grids) >= hard);
        System.out.println("MANHATTAN=" + manhattan(false, MODEL_GRIDS, grids));
        fillInGrids(N, this.rawList, this.orig_grids);
        System.out.println("RAWLIST " + rawList);
        System.out.println("INITIAL MAP IS SHOWN AS BELOW");
        dispGrids(this.grids);
    ...
    public static int manhattan(boolean cantor, int[][] MODEL_GRIDS, int[][] grids) {
        return calcManhattan(MODEL_GRIDS, grids);
    }
        ...
        for (int i = 0; i < MODEL_GRIDS.length; i++) {
            for (int j = 0; j < MODEL_GRIDS[i].length; j++) {
                if (MODEL_GRIDS[i][j] != 0) {
                    manhattan += Coordinate.distance(new Coordinate(j, i), fetchCoordinate(grids, MODEL_GRIDS[i][j]));
                }
            }
        }
        return manhattan;
    ...
    public static void main(String[] args) {
        ...
        main.generateRandomGame(4, 30);
        main.solve();
        
        main.generateRandomGame(5, 50);
        main.solve();
    }
}
